...
const orderProducts = async (orderedProducts) => {
  try {
    return await sequelize.transaction(async (transaction) => {
      const user = await userController.findUserByEmail("test@gmail.com");
      ...
      for (const product of products) {
        const orderProduct = orderedProducts.find(item => item.product_name === product.product_name);

        if (product.quantity === 0 ) {
          throw new Error(`${product.product_name} is out of stock`)
        }
        if (product.quantity < orderProduct.quantity) {
          throw new Error(`Inventory only has ${product.quantity} remaining for ${product.product_name}`);
        }
